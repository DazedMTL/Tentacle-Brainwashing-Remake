...
(function () {
  ...
  // パラメータの取得とバリデーション
  //=============================================================================
  var paramGameVariableTouchX = getParamNumber(
    ["GameVariableTouchX", "ポインタX座標の変数番号"],
    0
  );
  var paramGameVariableTouchY = getParamNumber(
    ["GameVariableTouchY", "ポインタY座標の変数番号"],
    0
  );
  var paramGameVariablePictNum = getParamNumber(
    ["GameVariablePictureNum", "ピクチャ番号の変数番号"],
    0
  );
  var paramTransparentConsideration = getParamBoolean([
    "TransparentConsideration",
    "透明色を考慮",
  ]);
  var paramSuppressTouch = getParamBoolean(["SuppressTouch", "タッチ操作抑制"]);
  var paramSuppressTouchSwitch = getParamNumber([
    "SuppressTouchSwitch",
    "タッチ操作抑制スイッチ",
  ]);
  var paramAlwaysCommonInBattle = getParamBoolean([
    "AlwaysCommonInBattle",
    "戦闘中常にコモン実行",
  ]);
  var paramInvalidSwitchId = getParamNumber(
    ["InvalidSwitchId", "無効スイッチ"],
    0
  );
  var paramAsParallelCommon = getParamBoolean([
    "AsParallelCommon",
    "並列処理として実行",
  ]);
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
    ...
    switch (getCommandName(command)) {
      ...
      case "ピクチャのボタン化":
        ...
        $gameScreen.setPictureCallCommon(
          pictureId,
          touchParam,
          trigger,
          transparent
  ...
  Game_Map.prototype.updatePictureCommonEvents = function () {
    ...
    this._pictureCommonEvents = this._pictureCommonEvents.filter(function (
      event
    ) {
  ...
  //  ピクチャがタッチされたときのコモンイベント呼び出し処理を追加定義します。
  //=============================================================================
  Game_Troop.prototype.setupPictureCommonEvent =
  ...
  Game_Screen.prototype.setPictureCallCommon = function (
    pictureId,
    touchParameter,
    trigger,
    transparent
  ...
  Game_Screen.prototype.setPictureStroke = function (
    pictureId,
    variableNum,
    transparent
  ) {
  ...
  var _Scene_Battle_updateBattleProcess =
  ...
  Spriteset_Base.prototype._iteratePicturesSub = function (
    containerChildren,
    callBackFund
  ) {
  ...
  Sprite_Picture.prototype.isIncludePointer = function () {
    return (
      this.isTouchable() && this.isTouchPosInRect() && !this.isTransparent()
    );
  };
  ...
  Sprite_Picture.prototype.callTouch = function () {
    ...
    for (var i = 0, n = this._triggerHandler.length; i < n; i++) {
      ...
      if (
        ...
        commandIds[i] &&
        handler.call(this) &&
        (this.triggerIsFocus(i) || !this.isTransparent())
      ) {
  ...
  Sprite_Picture.prototype.isTouchSuppress = function () {
    return paramSuppressTouchSwitch > 0
      ? $gameSwitches.value(paramSuppressTouchSwitch)
      : paramSuppressTouch;
  };
  ...
  Sprite_Picture.prototype.triggerIsFocus = function (triggerId) {
    return (
      this.triggerIsOnFocus(triggerId) || this.triggerIsOutFocus(triggerId)
    );
  };
  ...
  Sprite_Picture.prototype.isTouchable = function () {
    return (
      this.bitmap && this.visible && this.scale.x !== 0 && this.scale.y !== 0
    );
  };
  ...
  TouchInput._onMove = function (x, y) {
    if (this.isPressed())
      this._pressedDistance =
        Math.abs(this._prevX - x) + Math.abs(this._prevY - y);
    this._prevX = x;
    this._prevY = y;
